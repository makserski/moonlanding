function [xx,yy,zz] = lunar_orbits(varargin)
...
try
    myscale = 1737.1*Scale{2,strcmpi(Scale(1,:),units)};
catch %#ok<*CTCH>
...
sintheta = sin(angle); sintheta(1) = 0; sintheta(n+1) = 0;
x = myscale*cosphi*cos(angle);
...
if nargout == 0
    ...
    set(gca,'Color','k')
    hold on
t = linspace(0,2*pi,200) ;          %This produces an array of angle sizes for the circular orbit
R = (1848.220)*1 ;                  %Radius of the moon
x = R*cos(t) ;                      %Polar coordinate plot to produce an orbit plot
y = R*sin(t) ; 
comet(x,y) ;                        %Comet plot shows the progression of the translunar injection

r1 = (1737.200)*1;                  %This is the size of the perigee
r2 = (1849.730)*1;                  %This is the size of the apogee 
xt = r1 * cos(t);                   %This plots the coordinates in polar coordinates
yt = r2 * sin(t);

angle = pi/2;
cot = cos(angle); sit = sin(angle); %This applies the rotation of the angle
xa = xt * cot - yt * sit;           
yb = xt * sit - yt * cot;
% draw the curbe
comet(xa, yb);
